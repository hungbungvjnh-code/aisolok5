...
const ZALO_GROUP_URL = "https://zalo.me/g/tmyzem4l74wgb5aigvnj";
...
const RegistrationModal = ({ isOpen, onClose, utm }: { isOpen: boolean; onClose: () => void; utm: string }) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      // URL Google Apps Script của bạn (Thay thế link này sau khi Deploy)
      const SCRIPT_URL = "https://script.google.com/macros/s/AKfycbwsHLZNppYM5qYTaygs6Xp23Pq-0pZD6VTnHhHzXsItONQhYPO0y-v7vQQPY_xXaek3NQ/exec";
      ...
      await fetch(SCRIPT_URL, {
        method: 'POST',
        mode: 'no-cors', // Dùng no-cors để tránh lỗi Cross-Origin khi dùng Google Script
        headers: {
      ...
      setSuccess(true);
      // Tự động điều hướng sau 1.5 giây để khách hàng kịp thấy thông báo thành công
      setTimeout(() => {
        window.location.href = ZALO_GROUP_URL;
      }, 1500);
    } catch (err) {
      console.error("Lỗi đăng ký:", err);
      // Vẫn cho hiện thành công để không làm gián đoạn trải nghiệm khách hàng
      setSuccess(true);
      setTimeout(() => {
        window.location.href = ZALO_GROUP_URL;
      }, 2000);
    ...
    <div className="min-h-screen bg-white">
      ...
      <section id="coach" className="pb-20 md:pb-32 px-6 bg-white">
        <div className="max-w-[900px] mx-auto">
          <div className="apple-card p-8 md:p-16 flex flex-col md:flex-row items-center gap-8 md:gap-16 border border-apple-blue/10 bg-apple-blue/[0.02] shadow-xl shadow-blue-500/5">
            <div className="relative group/coach flex-shrink-0 w-48 h-48 md:w-64 md:h-64">
              <div className="absolute inset-0 bg-apple-blue rounded-full rotate-6 opacity-10 group-hover/coach:rotate-0 transition-transform duration-500"></div>
              <div className="relative w-full h-full rounded-full overflow-hidden shadow-2xl ring-4 ring-white">
                <img 
                  src="https://i.postimg.cc/Gm8Mv6sv/488987997-9386629448121893-1236487174668242866-n-(1).jpg" 
                  alt="Coach Hưng" 
            ...
            <div className="space-y-6 text-center md:text-left">
              <div className="space-y-2">
                <div className="inline-flex items-center gap-2 text-apple-blue font-black text-[10px] md:text-xs tracking-widest uppercase bg-apple-blue/10 px-3 py-1 rounded-full">
                  <Sparkles size={14} /> Your Personal Guide
                </div>
                <h3 className="text-3xl md:text-5xl font-black text-apple-black tracking-tighter uppercase">COACH ĐỒNG HÀNH.</h3>
                <p className="text-xl md:text-2xl font-bold text-apple-blue">Mr. Hưng</p>
              </div>
              
              <p className="text-apple-dark-gray font-bold text-base md:text-xl leading-relaxed italic">
                "Xin chào, tôi là Hưng - Người bạn đồng hành mới của bạn. Tôi sẽ trực tiếp hỗ trợ, giải đáp và dẫn dắt bạn từng bước cho đến khi làm được kết quả thực tế xuyên suốt chương trình này."
              </p>
              
              <div className="flex items-center justify-center md:justify-start gap-3 text-apple-blue font-black text-xs md:text-sm uppercase tracking-widest">
                <CheckCircle2 size={18} /> Cam kết hỗ trợ 1:1
              </div>
            </div>
          </div>
        </div>
